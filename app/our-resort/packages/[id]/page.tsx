...
export async function generateMetadata({ params }: { params: { id: string } }) {
  ...
  return {
    title: `${pkg.name} - Shanti Himalaya Resort Packages`,
    description: pkg.description || `Experience ${pkg.name} package at Shanti Himalaya Resort.`,
    keywords: [
      pkg.name,
      'resort packages',
    ...
    openGraph: {
      title: `${pkg.name} - Shanti Himalaya Resort`,
      description: pkg.description,
      url: `https://shantihimlaya.com/our-resort/packages/${pkg.slug}`,
      type: 'website',
      images: pkg.image_url ? [{ url: pkg.image_url, width: 1200, height: 630 }] : [],
    },
    twitter: {
...
Please provide more details about availability and booking.`
  ...
  const packageJSONLD = generateJSONLD(pkg, "Experience");
  const breadcrumbStructuredData = generateBreadcrumbJSONLD([
    { name: "Home", url: "/" },
    { name: "Our Resort", url: "/our-resort" },
    { name: "Packages", url: "/our-resort/packages" },
    { name: pkg.name, url: `/our-resort/packages/${pkg.slug}` },
  ]);
