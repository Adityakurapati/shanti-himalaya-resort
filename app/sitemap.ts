...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (destinationsRes.status === "fulfilled" && destinationsRes.value.data) {
      dynamicPages.push(
        ...destinationsRes.value.data.map((destination) => ({
          ...
          lastModified: destination.updated_at
    ...
    if (staysRes.status === "fulfilled" && staysRes.value.data) {
      dynamicPages.push(
        ...staysRes.value.data.map((stay) => ({
          ...
          lastModified: stay.updated_at
            ? new Date(stay.updated_at)
            : new Date(),
          changeFrequency: "monthly" as const,
          priority: 0.8,
        })),
    ...
    if (totalPages > URLS_LIMIT) {
      console.warn(
        `[Sitemap] Truncated from ${totalPages} to ${URLS_LIMIT} URLs (Google limit)`,
      );
    }
